            ...
            if (params.start || params.end) {
                return new Promise((resolve, reject) => {
                    ...
                    new (require('stream').Readable)({read() {
                        this.push(contents, params.encoding); this.push(null);
                    }}).pipe(stream);
    ...
    list(dirname) {
        return this.queue.run((slot) => {
            ...
            return fs.readdir(dirname)
                .then((files) => {
                    const stats = [];
                    return files.reduce((p, file) => p.then(() => fs.stat(path.posix.join(dirname, file)).then((stat) => {
                        stat.name = file; stats.push(stat);
