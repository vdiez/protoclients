...
const protocols = fs.readdirSync(path.posix.join(__dirname, './protocols'));
const clients = {};
...
module.exports = ({logger, protocol, params} = {}) => {
    const log = {info() {}, warn() {}, error() {}, verbose() {}, debug() {}};
    if (logger) {
    ...
    if (!protocols.includes(protocol + '.js')) throw 'Incorrect protocol';
    const client_id = get_class(protocol).generate_id(params);
...
module.exports.list = () => protocols.reduce((result, protocol) => {
    const extension = path.posix.extname(protocol);
